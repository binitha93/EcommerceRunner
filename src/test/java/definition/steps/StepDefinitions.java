...
public class StepDefinitions {
	...
	public void i_dismiss_login_popup() {
		try {
		WebElement loginpopupclose = wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.xpath(loginpopupcloseXpath))));
		if(loginpopupclose!=null)
	...
	public void i_check_the_checkbox(String filter) {
		...
		try {
			...
			try {
				...
				Assertions.fail("The filter '"+filterName+"' not available for selection");
			}
	}
	...
	public void the_search_should_return_corresponding_items() {
	   ...
	   for(WebElement ele:selectedFilters) {
		   Assertions.assertTrue(ele.getText().equalsIgnoreCase(filterName));
	   }
	}
